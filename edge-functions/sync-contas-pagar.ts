...
Deno.serve(async () => {
  ...
  try {
    ...
    for (const sit of [1, 2, 3]) {
      ...
      while (pageStart <= 20) {
        ...
        const results = await Promise.all(
          pages.map(p => blingFetch(`contas/pagar?pagina=${p}&limite=100&situacao=${sit}`, token))
        )
    ...
    await supabase.from('sync_log').insert({ tabela: 'contas_pagar', registros: total, status: 'ok', detalhes: duracao + 's' })
    return new Response(JSON.stringify({ ok: true, tabela: 'contas_pagar', registros: total, duracao_seg: duracao }), { headers: { 'Content-Type': 'application/json' }})
  } catch (e: any) {
    await supabase.from('sync_log').insert({ tabela: 'contas_pagar', registros: 0, status: 'error', erro: e.message })
